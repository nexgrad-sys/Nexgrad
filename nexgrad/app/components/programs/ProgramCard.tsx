      ...
      <div className="flex gap-6">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-6 text-sm text-gray-500 mb-4">
            ...
            <span>Mode: {program.mode?.replace("_", " ")}</span>

            {program.credits && <span>Credits: {program.credits} ECTS</span>}
          ...
          <div className="flex gap-4">
            <Link href={`/programs/${program.slug}`} className="btn-primary">
              View Details →
            </Link>

            <Link href={`/apply?program=${program.id}`} className="btn-outline">
              Apply Now
            </Link>
          </div>
        </div>
